...
-- 1. Fix RLS disabled on remaining tables
ALTER TABLE public.desarrollo_imagenes ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.dashboard_metrics_view ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.company_ventas_view ENABLE ROW LEVEL SECURITY;
...
-- Fix policies for dashboard_metrics_view
CREATE POLICY "Users can view own company dashboard metrics" 
ON public.dashboard_metrics_view 
FOR SELECT 
USING (
  EXISTS (
...
-- Fix policies for company_ventas_view
CREATE POLICY "Users can view own company ventas" 
ON public.company_ventas_view 
FOR SELECT 
USING (
  EXISTS (
    SELECT 1 FROM public.usuarios u 
