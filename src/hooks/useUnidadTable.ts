    ...
    if (isProcessingRef.current) {
      ...
      const operation = async () => {
        try {
          ...
          toast({
            title: "Unidad vendida",
            description: `La unidad ${currentUnidad.numero} ha sido marcada como vendida`
          });
          
          refetch();
        } catch (error: any) {
          console.error("Error selling unidad:", error);
          toast({
            title: "Error",
            description: `No se pudo vender la unidad: ${error.message}`,
            variant: "destructive"
          });
        }
      };
